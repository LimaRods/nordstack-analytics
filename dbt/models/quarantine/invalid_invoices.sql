-- Invoices breaching at least one data-quality rule, detected at the source.
--
-- Status is normalized INSIDE the rule expressions (lower/trim) because staging does
-- not repair it: an exact match on 'paid' would miss I000451's 'PAID ' and wrongly
-- clear it of the paid-amount checks.
--
-- excluded_from_marts separates two populations:
--   true  -> unattributable, or the amount is unusable: must not reach revenue
--   false -> usable; only the formatting is off (I000451), and the marts repair it
...
subscriptions as (
    ...
    -- un-deduplicated would double every invoice belonging to that subscription.
...
flagged as (
    ...
    select
        ...
        s.subscription_id is null                       as has_orphan_subscription,  -- D10
        (lower(trim(i.status)) = 'paid' and i.amount is null)
                                                        as has_null_paid_amount,     -- D11
        coalesce(i.amount <= 0, false)                   as has_non_positive_amount,  -- D12
        i.status <> lower(trim(i.status))                as has_unnormalized_status,  -- D13
        upper(trim(i.currency)) not in ('EUR', 'SEK')    as has_unsupported_currency, -- D14 guard
